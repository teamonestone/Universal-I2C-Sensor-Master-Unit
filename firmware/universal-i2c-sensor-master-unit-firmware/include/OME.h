...
private:
    ...
    static ObjInfo ObjInfos[OME_OBJ_POOL_SIZE];			    // The array with usage information.

    // functions
    static void* ConvertNoToPtr(int8_t poolNo);	            // Convert a pointer number in an actual pointer.
    static int8_t ConvertPrtToNo(void *objPtr);	            // Convert an actual pointer in a pointer number.
    static bool isInPoolRange(int8_t poolNo);				// Checks if the given pool object number is in the available memory space.
    static bool isInMemoryRange(void *objPtr);				// Checks if the given pointer is in the available memory space.
...
#endif //OBJECTMANAGEMENTENGINE_h